...
def check_pdf_producer(pdf_path, producer_strings=["TCPDF", "ReportLab"]):
    with open(pdf_path, 'rb') as file:
        reader = PdfReader(file)
        producer = reader.metadata.get('/Producer', '')
        return any(ps in producer for ps in producer_strings)
...
def convert_pdf_to_images(pdf_path, output_folder):
    ...
    for page_num in range(len(doc)):
        ...
        img = enhancer.enhance(2)  # Increase contrast, adjust factor as needed
...
def convert_pdf_to_ocr(source_file, destination_file):
    ...
    try:
        ocr_command = f'ocrmypdf "{source_file}" "{destination_file}" -l eng+hin -O 0 --force-ocr --deskew --clean --output-type pdf'
...
def process_pdfs_in_folder(source_folder, destination_folder, producer_strings=["TCPDF", "ReportLab"]):
